...
4. I assembled the generated code in NASM to produce a Linux executable.
5. I decompiled the assembled executable in Ghidra.
...
1. Copy a range of memory at symbol `M`, which looks like a series of `printf` format strings delimited by null terminators (`'\0'`), into `mem`.
...
The pause between entering the password and exiting indicates that some computation is done while `sprintf` processes the format strings. In [`read-fstrings.py`](https://github.com/kevin-he-01/gctf2020-sprint-writeup/blob/master/read-fstrings.py), I split the format strings into a list using the null byte `'\0'` as the delimiter, so the strings can be analyzed more easily.
...
Having `N$` immediately after `%` and `*` means that the format string specifier is explicitly referring to the `N`th variadic argument. For example, `%1$s` would print the first variadic argument as a string, and `%3$n` would write the number of characters written to an integer passed by reference in the third argument.
...
I assembled the generated program in NASM (for commands see [Makefile](https://github.com/kevin-he-01/gctf2020-sprint-writeup/blob/master/Makefile)) and ran it once to make sure it doesn't crash. I then loaded the binary in Ghidra \[TBD\]: (source code modified to increase clarity, such as turning characters to integer literals)
